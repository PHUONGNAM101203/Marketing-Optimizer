...
const GUIDE_STEPS: Readonly<Record<ProviderFamily, readonly string[]>> = {
  ...
  meta: [
    ...
    'Số liệu riêng của Trang Facebook (lượt hiển thị/tương tác Page, KHÁC Facebook Ads) cần quyền "read_insights". Danh sách bài đăng của Trang cần THÊM "pages_read_user_content" — "pages_read_engagement" không đủ cho edge này (xác nhận qua lỗi Graph API thật, mã #10: "This endpoint requires the pages_read_user_content permission or the Page Public Content Access feature"). Thêm cả hai quyền này ở đúng Use Case đang có "pages_show_list"/"pages_read_engagement" (mục "Quyền và tính năng" trên App Dashboard), Standard Access là đủ khi App còn ở Development Mode. Nếu App đã kết nối trước khi thêm quyền, phải NGẮT KẾT NỐI rồi kết nối lại Facebook — token cũ không tự có thêm quyền mới.',
